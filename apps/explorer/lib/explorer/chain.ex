defmodule Explorer.Chain do
  ...
  @default_paging_options %PagingOptions{page_size: 20}
  ...
  def address_to_logs(address_hash, options \\ []) when is_list(options) do
    paging_options = Keyword.get(options, :paging_options) || %PagingOptions{page_size: 20}
  ...
  *NOTE*
  ...
  def stream_unfetched_token_instances(initial, reducer) when is_function(reducer, 2) do
    nft_tokens =
      from(
        token in Token,
        where: token.type == ^"ARC-721"or token.type == ^"ARC-1155",
        select: token.contract_address_hash
      )
  ...
  @spec transaction_token_transfer_type(Transaction.t()) ::
          :arc20 | :arc721 | :arc1155 | :erc20 | :token_transfer | nil
  ...
  defp find_erc721_or_erc20_or_erc1155_token_transfer(token_transfers, {address, decimal_value}) do
    ...
    if token_transfer do
      case token_transfer.token do
        ...
        %Token{type: "ARC-20"} -> :arc20
        # %Token{type: "ERC-721"} -> :erc721
        # %Token{type: "ERC-1155"} -> :erc1155
        %Token{type: "ARC-721"} -> :arc721
        %Token{type: "ARC-1155"} -> :arc1155
        _ -> nil
      end
    else
      :arc20
