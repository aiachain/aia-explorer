defmodule BlockScoutWeb.Tokens.Helpers do
  ...
  defp do_token_transfer_amount(%Token{type: "ARC-20"}, nil, nil, _token_id, _token_ids) do
    {:ok, "--"}
  end

  defp do_token_transfer_amount(%Token{type: "ARC-20", decimals: nil}, amount, _amounts, _token_id, _token_ids) do
    {:ok, CurrencyHelpers.format_according_to_decimals(amount, Decimal.new(0))}
  end

  defp do_token_transfer_amount(%Token{type: "ARC-20", decimals: decimals}, amount, _amounts, _token_id, _token_ids) do
    {:ok, CurrencyHelpers.format_according_to_decimals(amount, decimals)}
  end
  ...
  # end
   ...
   defp do_token_transfer_amount(%Token{type: "ARC-721"}, _amount, _amounts, _token_id, _token_ids) do
    {:ok, :arc721_instance}
  end
  ...
  defp do_token_transfer_amount(%Token{type: "ARC-1155", decimals: decimals}, amount, amounts, _token_id, token_ids) do
    if amount do
      {:ok, :arc1155_instance, CurrencyHelpers.format_according_to_decimals(amount, decimals)}
    else
      {:ok, :arc1155_instance, amounts, token_ids, decimals}
    end
  end
